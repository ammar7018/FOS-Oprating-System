...
bool checkBit(int num, int bit) {
    return (num & (1 << bit));
}

void PrintBits(int address){
	for(int i=0;i<32;i++){
		if(checkBit(address,i)){
			cprintf("1");
		}else{
			cprintf("0");
		}
...
int nChanceClock_replacement(struct Env *curenv ){

	struct WorkingSetElement* ws_element=curenv->ptr_pageWorkingSet;
	...
	while(1){
		...
		for (unsigned int i=0;i<max_size;i++){
			...
			uint32 used_perm=pt_get_page_permissions(curenv,ws_element[i].virtual_address);

			if((used_perm & PERM_USED) ==  PERM_USED){
				pt_set_page_permissions(curenv, ws_element[i].virtual_address, 0, PERM_USED);
				ws_element[i].sweeps_counter=0;
			}
			//used bit=0
			else{
				//victim found,check modified bit

				ws_element[i].sweeps_counter++;
				if(ws_element[i].sweeps_counter>page_WS_max_sweeps){
					ws_element[i].sweeps_counter=page_WS_max_sweeps;
				}

				if(ws_element[i].sweeps_counter==page_WS_max_sweeps&&flag==-1){
					//modified bit=1,update & unmap

					uint32 modified_perm=pt_get_page_permissions(curenv,ws_element[i].virtual_address);
					if((modified_perm & PERM_MODIFIED ) == PERM_MODIFIED) {

						uint32 *ptr_page_table=NULL;//env page directory?
						get_page_table(curenv->env_page_directory,(void*)ws_element[i].virtual_address,&ptr_page_table);
						struct Frame_Info *ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)ws_element[i].virtual_address,&ptr_page_table);
						int ret = pf_update_env_page(curenv, (void*)ws_element[i].virtual_address, ptr_frame_info);
						pt_set_page_permissions(curenv, ws_element[i].virtual_address, 0, PERM_MODIFIED);

						unmap_frame(curenv->env_page_directory,(void*)ws_element[i].virtual_address);
//						env_page_ws_invalidate(curenv , ws_element[i].virtual_address);
						env_page_ws_clear_entry(curenv, i);
					}
					//modified bit=0,unmap
					else{
						unmap_frame(curenv->env_page_directory,(void*)ws_element[i].virtual_address);
//						env_page_ws_invalidate(curenv , ws_element[i].virtual_address);
						env_page_ws_clear_entry(curenv, i);
...
void placement(struct Env * curenv, uint32 fault_va){
		...
		int ret=map_frame(curenv->env_page_directory,ptr_frame,(void*)(fault_va),PERM_WRITEABLE|PERM_USER);
		...
		if(read_page==E_PAGE_NOT_EXIST_IN_PF)
		{
			if((fault_va<USTACKTOP)&&(fault_va>=USTACKBOTTOM)){
				int add_page=pf_add_empty_env_page(curenv,fault_va,0);
			}else{
				free_frame(ptr_frame);
				panic("I'm out");
			}
...
int replacement2(struct Env *curenv){


	while(1){

		int get_victim=-1;
		int size_ws=curenv->page_WS_max_size;

		for(int i=0;i<size_ws;i++){

			int va=curenv->ptr_pageWorkingSet[i].virtual_address;
			int all=pt_get_page_permissions(curenv,va);


			if((all&PERM_USED)==PERM_USED){
				...
				int cnt=curenv->ptr_pageWorkingSet[i].sweeps_counter;

				if(curenv->ptr_pageWorkingSet[i].sweeps_counter>page_WS_max_sweeps){
					curenv->ptr_pageWorkingSet[i].sweeps_counter=page_WS_max_sweeps;
					cnt=curenv->ptr_pageWorkingSet[i].sweeps_counter;
				}

				if(page_WS_max_sweeps<=cnt&&get_victim==-1){

					if((all&PERM_MODIFIED)==PERM_MODIFIED){

						uint32 *ptr_page=NULL;
						struct Frame_Info *ptr_frame_info=get_frame_info(curenv->env_page_directory,(void*)va,&ptr_page);
						pf_update_env_page(curenv, (void*)va, ptr_frame_info);
						pt_set_page_permissions(curenv, va, 0, PERM_MODIFIED);

					}

//					cprintf("Victim is %d ",i);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	//TODO: [PROJECT 2024 - MS1 - Fault handler] [5] page_fault_handler()
	...
	if(curenv->page_WS_max_size > env_page_ws_get_size(curenv)){
		...
		int idx=nChanceClock_replacement(curenv);
		placement(curenv,fault_va);
	}



	//TODO: [PROJECT 2024 - MS2 - BONUS3] Change WS Size according to Program Priority‌
}
