...
//	cprintf("Kernel base : %x\nGiven : %x\n",KERNEL_BASE,virtual_address);
	uint32 * b = kmalloc(PAGE_SIZE);
	if(b==0)
		return 0;
	uint32 pa = kheap_physical_address((uint32)b);
	if(pa==0)
		return 0 ;

//	pa/=PAGE_SIZE;
	//pa=pa|(PERM_AVAILABLE|PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
	uint32 entry = CONSTRUCT_ENTRY(pa,PERM_AVAILABLE|PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
...
{
	//Not Required: [PROJECT 2024 - [10] User Heap] allocateMem() [Kernel Side]
	// Write your code here, remove the panic and write your code
	panic("allocateMem() is not required...!!");
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//Not Required: [PROJECT 2024 - [12] User Heap] freeMem() [Kernel Side]
	// Write your code here, remove the panic and write your code
	panic("freeMem() is not required...!!");
...
{
	// Not Required: [PROJECT 2024 - BONUS3] User Heap Realloc [Kernel Side]
	//your code is here, remove the panic and write your code
...
void env_page_ws_print(struct Env *curenv)
{
	...
	for(i=0; i< (curenv->page_WS_max_size); i++ )
	{
		...
		cprintf(", used= %d, modified= %d, buffered= %d, sweep countern= %x", isUsed, isModified, isBuffered, curenv->ptr_pageWorkingSet[i].sweeps_counter) ;
