...
struct UserProgramInfo userPrograms[] = {
		...
		{ "tmlfq", "Tests CPU scheduling using MLFQ", PTR_START_OF(tst_CPU_MLFQ_master_1)},
...
void env_free(struct Env *e)
{
	//TODO: [PROJECT 2024 - MS1 - BONUS2] Exit [env_free()]
	...
	struct WorkingSetElement* ws=e->ptr_pageWorkingSet;

	for(int i=0;i<e->page_WS_max_size;i++){
		if(ws[i].empty==0){
			int perm = pt_get_page_permissions(e,ws[i].virtual_address);
			if((perm&PERM_USER)==PERM_USER)
			{
//				unmap_frame(e->env_page_directory,(void*)ws[i].virtual_address);
				kfree((void*)ws[i].virtual_address);
//				env_page_ws_clear_entry(e, i);
	...
	for(int i=0;i<PAGE_SIZE;i++){
		int entry=e->env_page_directory[i];
		if(((entry&PERM_USER)==PERM_USER)&&((entry&PERM_PRESENT)==PERM_PRESENT)){
			cprintf("I'm Here\n");
			entry=entry>>12;
			entry=entry*PAGE_SIZE;
			struct Frame_Info *table_frame_info = to_frame_info(entry);
			free_frame(table_frame_info);
		}
	}

//	unmap_frame(e->env_page_directory,(void*)e->ptr_pageWorkingSet);
...
//	env_page_ws_print(e);
//
//	struct WorkingSetElement* ws_element=e->ptr_pageWorkingSet;
//
//	for(int i=0;i<e->page_WS_max_size;i++){
//		if(ws_element[i].empty==0){
//
//			uint32 *ptr_page=NULL;
//			get_page_table(e->env_page_directory,(void*)ws_element[i].virtual_address,&ptr_page);
//
//			if(ptr_page==NULL){
//				return;
//			}
...
//				int *ptr=NULL;
//				struct Frame_Info *table_info=get_frame_info(e->env_page_directory,(void*)va,&ptr);
//				uint32 table_pa=to_physical_address(table_info);
//
...
//			}
//
//			uint32 dir_index = PDX((void*)ws_element[i].virtual_address);
//			ptr_page_directory[dir_index]=0;
//
//			unmap_frame(e->env_page_directory,(void*)ws_element[i].virtual_address);
//			env_page_ws_clear_entry(e, i);
//		}
//	}
//
