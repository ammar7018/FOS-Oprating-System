...
//assuming current env id doesn'r exceed 100000
//another solution we can put variable in struct of enviroment
//int current_env[100000];
...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	//TODO: [PROJECT 2024 - MS2 - [6] CPU Scheduling MLFQ] Initialize MLFQ
	...
	kclock_set_quantum(quantumOfEachLevel[0]);
}
...
{
	//TODO: [PROJECT 2024 - MS2 - [7] CPU Scheduling MLFQ] MLFQ Scheduler

//	lvl=lst_lvl;

	if(curenv!=NULL){

//		int lvl=current_env[curenv->env_id];
		if(lvl==num_of_ready_queues-1){
			enqueue(&(env_ready_queues[lvl]),curenv);
		}else{
			enqueue(&(env_ready_queues[lvl+1]),curenv);
//			current_env[curenv->env_id]++;
		}
